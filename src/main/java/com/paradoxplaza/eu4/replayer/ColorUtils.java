package com.paradoxplaza.eu4.replayer;

/**
 * A few methods for working with colors.
 */
public class ColorUtils {

    /** Color used in EU4 mapmode screenshots for seas. */
    static final public int SEA_COLOR = toColor(68, 107, 163);

    /** Color used in EU4 mapmode screenshots for wastelands. */
    static final public int WASTELAND_COLOR = toColor(94, 94, 94);

    /**
     * Creates a string "red green blue" out of int color.
     * The individual colors are integers 0-255.
     * @param color input color
     * @param separator delimiter to use
     * @return string "REDseparatorGREENseparator"
     */
    static public String colorToString255(final int color, final String separator) {
        final int red = (color & 0x00FF0000) >> 16;
        final int green = (color & 0x0000FF00) >> 8;
        final int blue = color & 0x000000FF;
        return red + separator + green + separator + blue;
    }

    /**
     * Creates a string "red green blue" out of int color.
     * The individual colors are floats 0-1.
     * @param color input color
     * @param separator delimiter to use
     * @return string "REDseparatorGREENseparator"
     */
    static public String colorToStringFloat(final int color, final String separator) {
        final double red = ((color & 0x00FF0000) >> 16) / 255.0;
        final double green = ((color & 0x0000FF00) >> 8) / 255.0;
        final double blue = (color & 0x000000FF) / 255.0;
        return red + separator + green + separator + blue;
    }

//    /**
//     * Returns whether the color could originate from color corruption.
//     * @param color color to check
//     * @return true if the color could originate from corruption, false otherwise
//     */
//    static public boolean isCorrupted(final int color) {
//        final int red = (color & 0x00FF0000) >> 16;
//        final int green = (color & 0x0000FF00) >> 8;
//        final int blue = color & 0x000000FF;
//        return red == 255 || green == 255 || blue == 255;
//    }
//
//    /**
//     * Checks whether the color potentionally colliding.
//     * Caused by Paradox bug interpreting 1 as 255 in country colors.
//     * @param color color to check
//     * @return true if the color is potentionally colliding, false otherwise
//     */
//    static public boolean isPotentionallyColliding(final int color) {
//        final int red = (color & 0x00FF0000) >> 16;
//        final int green = (color & 0x0000FF00) >> 8;
//        final int blue = color & 0x000000FF;
//        return red == 1 || red ==255
//                || green == 1 || green == 255
//                || blue == 1 || blue == 255;
//    }

    /**
     * R, G, B to argb.
     * @param red
     * @param green
     * @param blue
     * @return argb
     */
    static public int toColor(final int red, final int green, final int blue) {
        return 255 << 24 | red << 16 | green << 8 | blue;
    }

//    /**
//     * Transforms one color part to the corruption opposite.
//     * Ie. 1 changed to 255 and vice versa.
//     * @param color color to transform
//     * @return transformed color part
//     */
//    static private int transform(int color) {
//        if (color == 1) {
//            return 255;
//        } else if (color == 255) {
//            return 1;
//        } else {
//            return color;
//        }
//    }
//
//    /**
//     * Transforms color to colliding corruption opposite
//     * @param color color to transform
//     * @return transformed color
//     */
//    static public int toColliding(int color) {
//        final int red = (color & 0x00FF0000) >> 16;
//        final int green = (color & 0x0000FF00) >> 8;
//        final int blue = color & 0x000000FF;
//        return toColor(
//                transform(red), transform(green), transform(blue));
//    }
//
//    /**
//     * Uncorrupts the color. Ie. changes color parts from 255 to 1.
//     * @param color
//     * @return
//     */
//    static public int uncorrupt(final int color) {
//        final int red = (color & 0x00FF0000) >> 16;
//        final int green = (color & 0x0000FF00) >> 8;
//        final int blue = color & 0x000000FF;
//        return toColor(red == 255 ? 1 : red,
//                green == 255 ? 1 : green, blue == 255 ? 1 : blue);
//    }

    /** Utility classes need no constructors. */
    private ColorUtils() { }
}
